...
";
   ...
   /* public function load_block_emptyfields_table($tableid,$label='null',$table_index,$table_param,$type,$funzione='null',$recordid='null',$interface='desktop'){
    ...
    {
        $userid=$this->get_userid();
        if(($tableid=='calcolofatturato')&&($userid!=1))
        {
    ...
    public function load_block_results($tableid,$contesto=null,$query='',$master_tableid='',$page,$order_key='',$order_ascdesc='',$scrollTop=0,$limit_number=0)
    {   
...
*/
    ...
    public function load_3p_contratto_missione_content($funzione,$recordid_contratto='',$intestazione='true')
    {
        ...
        if($recordid_contratto>='00000000000000000000000000003583')
        {
            if($recordid_contratto>='00000000000000000000000000003586')
            {
    ...
    public function ajax_stampa_pdf_contratto_fornitura($recordid_contrattofornitura)
    {
        $content=  $this->stampa_pdf_contratto_fornitura($recordid_contrattofornitura);
    ...
    public function stampa_pdf_contratto_fornitura($recordid_contrattofornitura)
    {
        $data=array();
        $content=$this->load_3p_contratto_fornitura_content('stampa',$recordid_contrattofornitura);
        return $content;
    }
    ...
    public function load_3p_contratto_fornitura_content($funzione,$recordid_contrattofornitura='',$intestazione='true')
    {
        ...
        $data['recordid_contratto']=$recordid_contrattofornitura;
        $data['recordid_contrattofornitura']=$recordid_contrattofornitura;
        $data['contratto_fornitura']=$this->Sys_model->db_get_row('user_contrattofornitura',"*","recordid_='$recordid_contrattofornitura'");
    ...
    public function ajax_load_custom_3p_contratti_dipendente($recordid_presenzemensili)
    {
        $presenzemensili= $this->Sys_model->db_get_row('user_presenzemensili','*',"recordid_='$recordid_presenzemensili'");
        $recordid_dipendente=$presenzemensili['recordiddipendenti_'];
        echo $this->load_block_records_linkedtable('contratti','dipendenti',$recordid_dipendente,'modifica');
    }
    ...
    public function ajax_load_content_calcolofatturato_ccl($recordid_ccl,$anno='')
    {
        ...
        //$this->Sys_model->genera_calcolofatturato_ccl($recordid_ccl,$dati);
    ...
    public function ajax_report_ccl($recordid)
    {
        
        $content= $this->get_reportCCL($recordid);
        echo $content;
    }
    
    public function get_reportCCL($recordid,$print=false)
    {
        $userid=  $this->get_userid();
        $user_settings= $this->Sys_model->get_user_settings($userid);
        ...
        foreach ($qualifiche as $key => $qualifica) {
            ...
            $data['nrfascie']=count($fascie);
            $data['qualifiche'][$descrizione_qualifica][$recordid_qualifica]=$qualifica;
            $data['qualifiche'][$descrizione_qualifica][$recordid_qualifica]['fascie']=$fascie;
        }
    ...
    public function api_freshdesk_get_tickets()
    {
        ...
        $url = "https://$yourdomain.freshdesk.com/api/v2/tickets?include=requester,description,stats&updated_since=2020-07-01&per_page=100";
    ...
    public function api_bexio_get_orders()
    {
        //$oidc = new OpenIDConnectClient("https://idp.bexio.com", "a44f24cd-a2b4-4401-89bc-d7d53329accc", "AOoaD-UBBtBiWDGNFsQDClzQAtsI2XcpiSA3uJoDB7JbLF2t0hesG7cMbIDpv-hmrn-A6wZgsItC_-nQ0vsmQ7M");
        ...
        $client = new \GuzzleHttp\Client();
        $url = 'https://api.bexio.com/2.0/kb_order?order_by=updated_at_desc&limit=100';
    ...
    public function api_bexio_get_positions_test($token,$bexioorderid)
    {
        //$oidc = new OpenIDConnectClient("https://idp.bexio.com", "a44f24cd-a2b4-4401-89bc-d7d53329accc", "AOoaD-UBBtBiWDGNFsQDClzQAtsI2XcpiSA3uJoDB7JbLF2t0hesG7cMbIDpv-hmrn-A6wZgsItC_-nQ0vsmQ7M");
        //$oidc->setRedirectURL("http://localhost:8822/jdocweb/index.php/sys_viewcontroller/api_bexio_get_positions_test");
        ...
        //$token= $oidc->getAccessToken();
            ...
            $client = new \GuzzleHttp\Client();
            $url = 'https://api.bexio.com/2.0/kb_order/'.$bexioorderid.'/kb_position_custom';
            ...
            try {
                ...
                $json_response=($response->getBody()->getContents());
                $positions= json_decode($json_response,true);
                foreach ($positions as $key => $position) {
                    $bexioid=$position['id'];
                    $jdoc_ordine= $this->Sys_model->db_get_row('user_ordini','*',"bexioid='$bexioorderid'");
                    $jdoc_ordine_recordid=$jdoc_ordine['recordid_'];
                    $jdoc_ordine_ripetizione=$jdoc_ordine['ripetizione'];
                    $fields['bexioid']=$bexioid;
                    $fields['recordidordini_']=$jdoc_ordine_recordid;
                    $account_id=$position['account_id'];
                    $conto=$this->Sys_model->db_get_row('user_conti',"*","id='$account_id'");
                    $fields['conto']=$conto['numero'];
                    $fields['conto_descrizione']=$conto['descrizione'];
                    $fields['descrizione']=$position['text'];
                    $fields['prezzounitario']=$position['unit_price'];
                    $fields['quantita']=$position['amount'];
                    $fields['prezzototale']=$position['position_total'];
                    

                    $jdoc_row= $this->Sys_model->db_get_row('user_dettagliordine','*',"bexioid='$bexioid'");
                    if($jdoc_row!=null)
                    {
                        $recordid=$jdoc_row['recordid_'];
                        $this->Sys_model->update_record('dettagliordine',1,$fields,"recordid_='$recordid'");
                        echo("UPDATED ".$bexioid."<br/>");
                    }
                    else
                    {
                        $fields['id']= $this->Sys_model->generate_id('dettagliordine');
                        $this->Sys_model->insert_record('dettagliordine',1,$fields);
                        echo("INSERTED ".$bexioid."<br/>");
                    }
                    
                    if($this->isnotempty($jdoc_ordine_ripetizione))
                    {
                        if(($jdoc_ordine_ripetizione=='Annuale')||($jdoc_ordine_ripetizione=='Trimestrale')||($jdoc_ordine_ripetizione=='Mensile'))
                        {
                            if($conto['numero']=='3214')
                            {
                                $fields_ordine=array();
                                $fields_ordine['service']='153460';
                                $fields_ordine['servicecontracthours']=(int) ($position['position_total']/40);
                                $fields_ordine['servicecontracttype']='PBX';
                                $this->Sys_model->update_record('ordini',1,$fields_ordine,"recordid_='$jdoc_ordine_recordid'");
                            }
                            if($conto['numero']=='3218')
                            {
                                $fields_ordine=array();
                                $fields_ordine['service']='66466';
                                $fields_ordine['servicecontracthours']=(int) ($position['position_total']/40);
                                $fields_ordine['servicecontracttype']='BeAll';
                                $this->Sys_model->update_record('ordini',1,$fields_ordine,"recordid_='$jdoc_ordine_recordid'");
                            }
    ...
    public function custom_easywork_generadocumenti($converti='false')
    {
        ...
        $candidato= $this->Sys_model->db_get_row('V_Adi_L1001_Candidati','*',"ID='$candidato_id'");
        save_log("Candidato: ".json_encode($candidato));
        $candidato_progel= $this->Sys_model->db_get_row('V_Progel_tb_employee','*',"employee_number='$candidato_id'");
        save_log("Candidato Progel: ".json_encode($candidato_progel));
        $data['id']=$candidato_id;
        ...
        $datanascita=$candidato_progel['employee_birthDate'];
        if($this->isnotempty($datanascita)&&($datanascita!="1899-12-30 00:00:00.000"))
        {
            $data['datanascita_dd']=date('d', strtotime($datanascita));
            $data['datanascita_mm']=date('m', strtotime($datanascita));
            $data['datanascita_yyyy']=date('Y', strtotime($datanascita)); 
        }
        else
        {
            $data['datanascita_dd']="";
        ...
        if(($genere==2)||($genere=='2'))
        {
            $data['sesso_f']="X";
        }
        $data['nazione_i']="X";
        $data['telefono']=$candidato['Cellulare'];
        $statocivile=$candidato_progel['employee_civilStatus'];
        $data['sc1']="";
        $data['sc3']="";
        $data['sc6']="";
        if($statocivile==1)
        ...
        if(($statocivile!=1)&&($statocivile!=3))
        {
            $data['sc6']="X";
        }
        $datamatrimonio=$candidato_progel['employee_marriageDate'];
        $dataseparazione=$candidato_progel['employee_customDataSeparazione'];
        $datadivorzio=$candidato_progel['employee_customDataDivorzio'];
        $datavedovanza=$candidato_progel['employee_customDataVedovanza'];
        $datastatocivile=$candidato_progel['employee_civilStatusValidFrom'];
        if($this->isnotempty($datastatocivile)&&($datastatocivile!="1899-12-30 00:00:00.000"))
        {
            $data['datastatocivile_dd']=date('d', strtotime($datastatocivile));
            $data['datastatocivile_mm']=date('m', strtotime($datastatocivile));
            $data['datastatocivile_yyyy']=date('Y', strtotime($datastatocivile));
        }
        else
        {
            $data['datastatocivile_dd']="";
            $data['datastatocivile_mm']="";
            $data['datastatocivile_yyyy']="";
            
        }
        
        
        //sposa - sposo
        $data['spcognome']=$candidato_progel['employee_spouseLastName'];
        $data['spnome']=$candidato_progel['employee_spouseFirstName'];
        $data['spnumeroavs']=$candidato_progel['employee_spouseSocialInsuranceNumber'];
        $spdatanascita=$candidato_progel['employee_spouseBirthDate'];
        if($this->isnotempty($spdatanascita)&&($spdatanascita!="1899-12-30 00:00:00.000"))
        {
            $data['spdatanascitadd']=date('d', strtotime($spdatanascita));
            $data['spdatanascitamm']=date('m', strtotime($spdatanascita));
            $data['spdatanascitayyyy']=date('Y', strtotime($spdatanascita));
        }
        else
        {
            $data['spdatanascitadd']="";
            $data['spdatanascitamm']="";
            $data['spdatanascitayyyy']="";
        }
        $data['spindirizzo']=$candidato_progel['employee_spouseAddress'];
        $data['spcap']=$candidato_progel['employee_spouseAddressPostalCode'];
        $data['sppaese']=$candidato_progel['employee_swissLocality'];
        $spdataattivita=$candidato_progel['employee_spouseEmploymentStartDate'];
        if($this->isnotempty($spdataattivita)&&($spdataattivita!="1899-12-30 00:00:00.000"))
        {
            $data['spdataattivitadd']=date('d', strtotime($spdataattivita));
            $data['spdataattivitamm']=date('m', strtotime($spdataattivita));
            $data['spdataattivitayyyy']=date('Y', strtotime($spdataattivita));  
        }
        else
        {
            $data['spdataattivitadd']="";
            $data['spdataattivitamm']="";
            $data['spdataattivitayyyy']="";
        }
        
        if($this->isnotempty($data['spnumeroavs']))
        {
            $data['spredditoestero']="";
            $data['spredditosvizzera']="X";
        }
        else
        {
            $data['spredditoestero']="X";
            $data['spredditosvizzera']="";
        }
        
        //figlio 1
        $data['f1nome']=$candidato_progel['employee_customFiglioNome'];
        $data['f1cognome']=$candidato_progel['employee_customFiglioCognome'];
        $f1datadinascita=$candidato_progel['employee_customFiglioDataDiNascita'];
        if($this->isnotempty($f1datadinascita)&&($f1datadinascita!="1899-12-30 00:00:00.000"))
        {
            $data['f1datadinascitadd']=date('d', strtotime($f1datadinascita));
            $data['f1datadinascitamm']=date('m', strtotime($f1datadinascita));
            $data['f1datadinascitayyyy']=date('Y', strtotime($f1datadinascita));  
        }
        else
        {
            $data['f1datadinascitadd']="";
            $data['f1datadinascitamm']="";
            $data['f1datadinascitayyyy']="";
        }
        
        
        
        //figlio 2
        $data['f2nome']=$candidato_progel['employee_customFiglioNome001'];
        $data['f2cognome']=$candidato_progel['employee_customFiglioCognome001'];
        $f2datadinascita=$candidato_progel['employee_customFiglioDataDiNascita001'];
        if($this->isnotempty($f2datadinascita)&&($f2datadinascita!="1899-12-30 00:00:00.000"))
        {
            $data['f2datadinascitadd']=date('d', strtotime($f2datadinascita));
            $data['f2datadinascitamm']=date('m', strtotime($f2datadinascita));
            $data['f2datadinascitayyyy']=date('Y', strtotime($f2datadinascita));  
        }
        else
        {
            $data['f2datadinascitadd']="";
            $data['f2datadinascitamm']="";
            $data['f2datadinascitayyyy']="";
        }
        
        
        
        //figlio 3
        $data['f3nome']=$candidato_progel['employee_customFiglioNome002'];
        $data['f3cognome']=$candidato_progel['employee_customFiglioCognome002'];
        $f3datadinascita=$candidato_progel['employee_customFiglioDataDiNascita002'];
        if($this->isnotempty($f3datadinascita)&&($f3datadinascita!="1899-12-30 00:00:00.000"))
        {
            $data['f3datadinascitadd']=date('d', strtotime($f3datadinascita));
            $data['f3datadinascitamm']=date('m', strtotime($f3datadinascita));
            $data['f3datadinascitayyyy']=date('Y', strtotime($f3datadinascita));  
        }
        else
        {
            $data['f3datadinascitadd']="";
            $data['f3datadinascitamm']="";
            $data['f3datadinascitayyyy']="";
        }
        ...
        {
            $data['altreattivitasi']="X";
            $data['altreattivitano']="";
        }
        else
        {
        ...
        foreach ($documenti as $key => $documento) {
            ...
            if($ext=='docx')
            {
                ...
                save_log("Genere $cognomenome: $genere");
                if(($genere==2)||($genere=='2'))
                {
                    $data['modello']=$documentonoext."_donna.docx";
                }
                save_log($data['modello']);
    ...
    public function ajax_calendarioaziendale($recordid_azienda,$anno)
    {
        $calendarioaziendale=$this->Sys_model->get_calendarioaziendale($recordid_azienda,$anno);
